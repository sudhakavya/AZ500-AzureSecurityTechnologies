...
# Lab 01: Key Vault (Implementing Secure Data by setting up Always Encrypted)
# Student lab manual
...
1. On the **Custom deployment** blade, under **Deployment Scope** ensure that the following settings are configured (leave any others with their default values):
   ...
   |Resource group|click **Create new** and type the name **AZ500LAB01**|
   |Location|**(US) East US**|
...
1. In the PowerShell session within the Cloud Shell pane, run the following to create an Azure Key Vault in the resource group **AZ500LAB01**. (If you chose another name for this lab's Resource Group out of Task 1, use that name for this task as well). The Key Vault name must be unique. Remember the name you have chosen. You will need it throughout this lab.  

    ```powershell
    $kvName = 'az500kv' + $(Get-Random)

    $location = (Get-AzResourceGroup -ResourceGroupName 'AZ500LAB01').Location

    New-AzKeyVault -VaultName $kvName -ResourceGroupName 'AZ500LAB01' -Location $location
    ```
...
1. On the **Resource groups** blade, in the list of resource group, click the **AZ500LAB01** (or other name you chose earlier for the resource group) entry.
...
1. In the PowerShell session within the Cloud Shell pane, run the following to add a software-protected key to the Key Vault: 
    ...
    ```powershell
    $kv = Get-AzKeyVault -ResourceGroupName 'AZ500LAB01'
...
1. In the PowerShell session within the Cloud Shell pane, run the following to create a variable storing the Key Vault name.
	```
    $kvName = (Get-AzKeyVault -ResourceGroupName 'AZ500LAB01').VaultName
...
1. In the PowerShell session within the Cloud Shell pane, run the following to grant permissions on the Key Vault to the application you registered in the previous task:
    ...
    ```powershell
    Set-AZKeyVaultAccessPolicy -VaultName $kvName -ResourceGroupName AZ500LAB01 -ServicePrincipalName $applicationId -PermissionsToKeys get,wrapKey,unwrapKey,sign,verify,list
    ```
...
1. In the list of SQL databases, click the **medical(<randomsqlservername>)** entry.
    ...
    >**Note**: If the database cannot be found, this likely means the deployment you initiated in Exercise 1 has not completed yet. You can validate this by browsing to the Azure Resource Group "AZ500LAB01" (or the name you chose), and selecting **Deployments** from the Settings pane.  
...
>**Note**: Before you proceed with this task, ensure that the deployment you initiated in the first exercise has completed successfully. You can validate this by navigating to the blade of the Azure resource group "Az500Lab01" (or other name you chose) and selecting **Deployments** from the Settings pane.  
